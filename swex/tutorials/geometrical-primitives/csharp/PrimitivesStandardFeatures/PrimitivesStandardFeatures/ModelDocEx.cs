...
namespace SolidWorks.Interop.sldworks
{
    ...
    {
        public static void CreateBox(this IModelDoc2 model, double width, double length, double height)
        {
            CreateExtrudedPrimitive(model, skMgr => skMgr.CreateCenterRectangle(0, 0, 0, width / 2, height / 2, 0) != null, height);
        }

        public static void CreateCylinder(this IModelDoc2 model, double diam, double height)
        {
            CreateExtrudedPrimitive(model, skMgr => skMgr.CreateCircleByRadius(0, 0, 0, diam / 2) != null, height);
        }

        private static void CreateExtrudedPrimitive(IModelDoc2 model, Func<ISketchManager, bool> creator, double height)
        {
            try
            {
