...
Module ModelDocEx
	...
	<Extension>
	Sub CreateBox(ByVal model As IModelDoc2, ByVal width As Double, ByVal length As Double, ByVal height As Double)
		CreateExtrudedPrimitive(model, Function(skMgr) skMgr.CreateCenterRectangle(0, 0, 0, width / 2, height / 2, 0) IsNot Nothing, height)
	End Sub

	<Extension>
	Sub CreateCylinder(ByVal model As IModelDoc2, ByVal diam As Double, ByVal height As Double)
		CreateExtrudedPrimitive(model, Function(skMgr) skMgr.CreateCircleByRadius(0, 0, 0, diam / 2) IsNot Nothing, height)
	End Sub

	Private Sub CreateExtrudedPrimitive(ByVal model As IModelDoc2, ByVal creator As Func(Of ISketchManager, Boolean), ByVal height As Double)
		Try
	...
	Private Function CreateSketch(ByVal model As IModelDoc2, ByVal creator As Func(Of ISketchManager, Boolean)) As Sketch
		...
		If Not creator.Invoke(skMgr) Then
